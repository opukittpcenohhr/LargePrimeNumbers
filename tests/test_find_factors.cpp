...
TEST(TrialFindFactorTest, test) {
  ...
  test_find_factor(
      find_factor_trial, "find_factor_trial", exclusions, std::nullopt, 10);
}
...
TEST(PollardRhoFindFactorTest, test) {
  ...
  test_find_factor(
      test_find_factor_pollard_rho, "find_factor_pollard_rho", kExclusions,
      std::nullopt, kBigNumberThreshold);
}
...
TEST(PollardRhoOneFindFactorTest, test) {
  ...
  test_find_factor(
      test_find_factor_pollard_rho_one, "find_factor_pollard_rho_one",
      kExclusions, std::nullopt, kBigNumberThreshold);
}

TEST(QuadraticSieveFactorTest, test) {
  constexpr auto kTestParams = QuadraticSieveParams{5000, 100000000, 2.0};
  const std::set<bigint> kExclusions = {};
  ...
  test_find_factor(
      test_find_factor_quadratic_sieve, "find_factor_quadratic_sieve",
      kExclusions, kBigNumberThreshold, std::nullopt);
}

TEST(CfracFactorTest, test) {
  constexpr auto kTestParams = CfracParams{500};
  const std::set<bigint> kExclusions = {};

  auto test_find_factor_cfrac = [&kTestParams](const bigint &n) {
    return find_factor_cfrac(n, kTestParams);
  };

  test_find_factor(
      test_find_factor_cfrac, "find_factor_cfrac", kExclusions,
