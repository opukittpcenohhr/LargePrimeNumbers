...
inline std::vector<int> find_factor_base(bigint n, size_t factors_count) {
  ...
  std::vector<bool> is_prime(max_number_to_check + 1, 1);
  for (size_t i = 2; i <= max_number_to_check; i++) {
    if (is_prime[i]) {
